...
export class ProductsController {
  ...
  @Get(':productId')
  @HttpCode(HttpStatus.OK)
  async findOne(@Param('productId') productId: string): Promise<{message: string, data: Product}> {
  ...
  @Patch(':productId')
  @HttpCode(HttpStatus.OK)
  async update(@Param('productId') productId: string, @Body() dto: UpdateProductDto): Promise<{message: string, data: Product}> {
    const updatedProduct = await this.productsService.updateProduct(productId, dto);
    return {message: 'Product has updated successfully', data: updatedProduct}
  }

  @Delete(':productId')
  @HttpCode(HttpStatus.NO_CONTENT)
  async remove(@Param('productId') productId: string): Promise<{message: string}> {
