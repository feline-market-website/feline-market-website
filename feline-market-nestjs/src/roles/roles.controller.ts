...
export class RolesController {
  ...
  async create(@Body() createRoleDto: CreateRoleDto): Promise<{message: string, data: Role}> {
    const role = await this.roleService.create(createRoleDto);
    return {message: `Role ${role.name} has created successfully`, data: role}
  }
  ...
  @Get(':id')
  @HttpCode(HttpStatus.OK)
  async findOne(@Param('id') id: string): Promise<{message: string, data: Role}> {
